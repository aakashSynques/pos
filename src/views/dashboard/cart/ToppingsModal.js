  ...
  return (
    <div className="topping-model">
      <div className="topping-header">
        {/* Header content for the topping model */}
        <CCol sm={4}>
          <CModalTitle>Apply Toppings</CModalTitle>
        </CCol>
        <CCol sm={8}>
          ...
          <label key={topping.prod_id}>
            ...
            >
              ...
              <span className="show_price">
                {topping.rate_chart[selectedOutletId][0].prod_rate}
              </span>
            </button>
          </label>
        ))}
      </div>
      <div className="topping-footer">
        {/* Footer content for the topping model */}
        <button onClick={clearAllToppings}>Clear All [Alt + C]</button>
        <button onClick={handleToppingsSubmit}>Submit [Alt + Enter]</button>
        <button onClick={() => setToppingModel(false)}>Cancel [Esc]</button>
      </div>
    </div>
  );
};
